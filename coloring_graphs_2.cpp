...
int main () {
    ...
    while( T-- ) {
        ...
        Matrix<100> trans ;
        for( int i=0; i<n;i++ ) trans.v[i][i] = 1;
        ...
        for( int i=r-1; i >= 1; i-- ) {
            ...
            for( int j=0; j<n; j++ )
                for( int k=0; k<n; k++ )
                    cur.v[j][k] = _mat[i][j][k];
            trans2 = trans2.mul( cur );
            trans2 = trans2.mod( MOD );
        }

        // power the transition matrix

        while(pw){
            if(pw&1) trans = trans.mul( trans2 );
            trans2=trans2.mul( trans2 );
            pw>>=1;
        }
